...
void flush(uint32_t dir_cluster) {
    ...
    memcpy(&temp, &driver.fat_table, sizeof(struct FAT32FileAllocationTable));
    write_clusters(&temp, FAT_CLUSTER_NUMBER, 1);
    memset(&temp, 0, CLUSTER_SIZE);
    memcpy(&temp, &driver.dir_table_buf, sizeof(struct FAT32DirectoryTable));
    write_clusters(&temp, dir_cluster, 1);
}
...
int8_t write(struct FAT32DriverRequest request) {
    ...
    if (dir_table->table[empty_entry].attribute == ATTR_SUBDIRECTORY) {
